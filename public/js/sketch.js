...
let button, input; // create variables for button and input field

let sliderLat, sliderLong; // make couple variables for our sliders
let ballArray = []; //create an array for our ball objects
...
var URL = 'https://api.darksky.net/forecast/'; // base URI for the API call
...
function draw() {
    ...
    for (let i = 0; i < ballArray.length; i++) { // because loadData & subsequently gotData are  is called in setup an our program already knows how long the array is. 
        ballArray[i].show(); 
        ballArray[i].update();
        ballArray[i].move();
    }
...
function gotData(d) {
    ...
    ballArray = []; // clear the array when we get new data 
    ...
    humid = map(d.currently.humidity, 0, 1, 10, 0);
   

    for (let i = 0; i < d.currently.temperature; i++ ){ // take the temperature variable and make our array that size 

        ballArray[i] = new ball( 320, 160, i, i*2, 5); // create new ball objects, according to the size of the array
