...
function keyPressed() {
    ...
    if (keyIsDown(49)) {
        ...
        currentMode = 1; // sun mode
    } else if (keyIsDown(51)) {
        currentMode = 2; // rain mode
    } else if (keyIsDown(52)) {
        currentMode = 3; // wind mode
    }
    else if (keyIsDown(53)) {
        currentMode = 4; // cloudy mode
    } else {
        currentMode = 0; // wait mode
    }
...
function gotData(d) {
    ...
    let windSpeed = createP('windspeed : ' + d.currently.windSpeed); // create a variable for the windspeed text string and create a paragraph in it with the data
    windSpeed.parent('windSpeedDiv'); //assign the windspeed data container to the existing div with the id windSpeed

    let humidity= createP('humidity : '+ d.currently.humidity);
    humidity.parent('humidityDiv');

    let temperature = createP("temperature : " + d.currently.temperature);
    temperature.parent('temperatureDiv');

    let summary = createP('summary : '+d.currently.summary);
    summary.parent('summaryDiv');
